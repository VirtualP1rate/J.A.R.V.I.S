...
# In-process loopback base for agent tools that call J.A.R.V.I.S's own API
...
def _internal_headers(owner: Optional[str] = None) -> Dict[str, str]:
    ...
    if owner:
        headers["X-J.A.R.V.I.S-Owner"] = owner
    return headers
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to allowed internal J.A.R.V.I.S API endpoints. Lets the
    ...
    for t in merged:
        ...
        if t.get("status") == "error" and t.get("output_tail"):
            ...
            if tail:
                ...
                # banner ("Locale: C / Ubuntu_J.A.R.V.I.S ❯") — useless for
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
            ...
            for t in (state.get("tasks") or []):
                if isinstance(t, dict) and (t.get("sessionId") == session_id or t.get("id") == session_id):
                    ...
                    if not sport:
                        sport = t.get("sshPort") or ""
                    break
    # Prefer the persisted /tmp/jarvis-tmux/SESSION.log file over the
    ...
    log_path = f"/tmp/jarvis-tmux/{session_id}.log"
