...
def _apply_jarvis_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-J.A.R.V.I.S-Origin"] = JARVIS_MAIL_ORIGIN
    if kind:
        msg["X-J.A.R.V.I.S-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-J.A.R.V.I.S-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the J.A.R.V.I.S marker header, but include the subject
                # fallback too. The fallback uses a distinct J.A.R.V.I.S prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-J.A.R.V.I.S-Kind "reminder" SUBJECT "Reminder (J.A.R.V.I.S):"{from_clause})',
                )
    ...
    async def delete_jarvis_reminder_emails(
        ...
        """Delete email messages stamped as J.A.R.V.I.S reminders."""
        if account_id:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-J.A.R.V.I.S-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (J.A.R.V.I.S):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (J.A.R.V.I.S):")})'))
                            # Legacy reminders created before the J.A.R.V.I.S
