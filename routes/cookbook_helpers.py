...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When J.A.R.V.I.S runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends J.A.R.V.I.S' own interpreter directory to PATH. If J.A.R.V.I.S itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[jarvis] pip does not support --break-system-packages; installing without it."')
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by J.A.R.V.I.S.
    """
    if not cmd:
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for J.A.R.V.I.S-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('JARVIS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$JARVIS_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  JARVIS_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[jarvis] vLLM CLI: $JARVIS_VLLM_BIN"')
    runner_lines.append('  JARVIS_VLLM_VERSION="$("$JARVIS_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$JARVIS_VLLM_VERSION" ]; then echo "[jarvis] vLLM version: $JARVIS_VLLM_VERSION"; fi')
    runner_lines.append('fi')
