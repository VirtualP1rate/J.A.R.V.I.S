...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the J.A.R.V.I.S container by design. The image ships no "
...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside J.A.R.V.I.S."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside J.A.R.V.I.S.",
        )
