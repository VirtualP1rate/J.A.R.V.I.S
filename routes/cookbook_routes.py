...
_HF_TOKEN_STATUS_SNIPPET = (
    ...
    'Add one in J.A.R.V.I.S Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When J.A.R.V.I.S runs from a venv (e.g. native macOS install), put its bin
        ...
        if is_ollama_download:
            ...
            lines.append(f'  JARVIS_OLLAMA_PULL_CMD={shlex.quote(ollama_cmd)}')
            lines.append('elif command -v docker >/dev/null 2>&1; then')
            lines.append('  JARVIS_OLLAMA_CONTAINER="$(docker ps --format \'{{.Names}}\' 2>/dev/null | grep -E \'^(ollama-rocm|ollama-test)$\' | head -1)"')
            lines.append('  if [ -n "$JARVIS_OLLAMA_CONTAINER" ]; then')
            lines.append(f'    JARVIS_OLLAMA_PULL_CMD={shlex.quote("docker exec ${JARVIS_OLLAMA_CONTAINER} " + ollama_cmd)}')
            lines.append('  fi')
            lines.append('fi')
            lines.append('if [ -z "$JARVIS_OLLAMA_PULL_CMD" ]; then echo "ERROR: Ollama not found on this server. Install Ollama or start an ollama-rocm/ollama-test container."; exit 127; fi')
        else:
        ...
        if remote and is_windows:
            ...
            if is_ollama_download:
                ...
                runner_lines.append(f'  JARVIS_OLLAMA_PULL_CMD={shlex.quote(ollama_cmd)}')
                runner_lines.append('elif command -v docker >/dev/null 2>&1; then')
                runner_lines.append('  JARVIS_OLLAMA_CONTAINER="$(docker ps --format \'{{.Names}}\' 2>/dev/null | grep -E \'^(ollama-rocm|ollama-test)$\' | head -1)"')
                runner_lines.append('  if [ -n "$JARVIS_OLLAMA_CONTAINER" ]; then')
                runner_lines.append(f'    JARVIS_OLLAMA_PULL_CMD={shlex.quote("docker exec ${JARVIS_OLLAMA_CONTAINER} " + ollama_cmd)}')
                runner_lines.append('  fi')
                runner_lines.append('fi')
                runner_lines.append('if [ -z "$JARVIS_OLLAMA_PULL_CMD" ]; then echo "ERROR: Ollama not found on this server. Install Ollama or start an ollama-rocm/ollama-test container."; exit 127; fi')
            else:
            ...
            if is_ollama_download:
                runner_lines.append('  eval "$JARVIS_OLLAMA_PULL_CMD" < /dev/null')
            else:
            ...
            _hf_invoke = 'eval "$JARVIS_OLLAMA_PULL_CMD" < /dev/null' if is_ollama_download else (hf_cmd if IS_WINDOWS else f"{hf_cmd} < /dev/null")
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python J.A.R.V.I.S is already
    ...
    def _auto_register_llm_endpoint(req: ServeRequest, remote: str | None) -> str | None:
        ...
        # the jarvis container — so the right URL for the in-container
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines.append("mkdir -p /tmp/jarvis-tmux 2>/dev/null || true")
            runner_lines.append("exec 3>&1 4>&2")
            runner_lines.append(
                f"exec > >(tee -a /tmp/jarvis-tmux/{session_id}.log) 2>&1"
            )
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('JARVIS_PREFLIGHT_EXIT=""')
            # Put J.A.R.V.I.S's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('  _ody_try_port=$((JARVIS_OLLAMA_PORT + _ody_off))')
                runner_lines.append('  if ! (exec 3<>/dev/tcp/127.0.0.1/$_ody_try_port) 2>/dev/null; then')
                runner_lines.append('    exec 3<&-; exec 3>&-')
                runner_lines.append('    JARVIS_OLLAMA_PORT="$_ody_try_port"')
                ...
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[jarvis] WARNING: remote Ollama will bind to ${JARVIS_OLLAMA_HOST}:${JARVIS_OLLAMA_PORT} so J.A.R.V.I.S can reach it from this host."')
            ...
            if (not handled_ollama_serve
                ...
                runner_lines.append('  echo "[jarvis] Ollama sidecar model is available; keeping Cookbook task attached to the persistent Ollama daemon."')
            ...
            try:
                async with _httpx.AsyncClient(timeout=8, follow_redirects=True) as client:
                    resp = await client.get(
                        ...
                        headers={"User-Agent": "jarvis-cookbook/1.0"},
                    )
                if resp.status_code == 200:
                    html = resp.text
    ...
    def _cookbook_tasks_status_sync():
        ...
        for task in tasks:
            ...
            if task_platform == "windows" and remote:
                ...
                # as just "Locale: C / Ubuntu_J.A.R.V.I.S ❯" and the agent
