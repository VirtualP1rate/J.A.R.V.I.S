#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for J.A.R.V.I.S.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/J.A.R.V.I.S.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/J.A.R.V.I.S.dmg   — drag-to-Applications disk image (the downloadable).
#
...
#!/bin/bash
# J.A.R.V.I.S.app — start the local server and open the UI in an app window.
...
notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"J.A.R.V.I.S\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"J.A.R.V.I.S\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "J.A.R.V.I.S isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "J.A.R.V.I.S failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "J.A.R.V.I.S is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag J.A.R.V.I.S to Applications)"
