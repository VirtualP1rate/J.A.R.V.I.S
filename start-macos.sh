#!/bin/bash
# J.A.R.V.I.S — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything J.A.R.V.I.S needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine J.A.R.V.I.S
...
echo "▶ J.A.R.V.I.S quick start for macOS"
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/jarvis-apfel.log"
    ...
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ J.A.R.V.I.S is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting J.A.R.V.I.S — it will open in your browser at $URL"
