#!/usr/bin/env python3
"""Small J.A.R.V.I.S scoped API helper for Codex terminal sessions."""
...
def _usage() -> int:
    ...
    print("  jarvis_api.py cookbook tasks", file=sys.stderr)
    print("  jarvis_api.py cookbook servers", file=sys.stderr)
    print("  jarvis_api.py cookbook cached [HOST]", file=sys.stderr)
    print("  jarvis_api.py cookbook presets", file=sys.stderr)
    print("  jarvis_api.py cookbook output SESSION_ID [tail]", file=sys.stderr)
    print("  jarvis_api.py cookbook serve REPO_ID 'CMD' [REMOTE_HOST]", file=sys.stderr)
    print("  jarvis_api.py cookbook preset NAME", file=sys.stderr)
    print("  jarvis_api.py cookbook adopt SESSION_ID MODEL [HOST] [PORT]", file=sys.stderr)
    print("  jarvis_api.py cookbook stop SESSION_ID", file=sys.stderr)
...
def _config() -> tuple[str, str] | None:
    ...
    if missing:
        print(f"missing {', '.join(missing)}; create a Codex Agent token in J.A.R.V.I.S Settings", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    if not path.startswith("/api/codex/"):
        print("refusing non-/api/codex path; use scoped J.A.R.V.I.S integration endpoints only", file=sys.stderr)
        return 2
